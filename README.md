...
| **Snake** | ゲーム |
| **Music** | バックグラウンド再生デモ。アプリを閉じても曲が続く(別タスクで再生) |
| **Tasks** | プロセスモニタ。ジョブの進捗/スタック残量表示、K でキャンセル、X で強制kill |
| **Config** | 明るさ・音量・キークリック音・テーマ(dark/light)・TZ などを NVS に永続化 |
| **Info** | チップ・メモリ・ストレージ・ネットワークのシステム情報 |

### マルチタスク(v0.2.0)

UIはシングルスレッドのまま、バックグラウンドジョブを本物の FreeRTOS タスクとして実行する
ハイブリッド構成(AndroidのメインスレッドとWorkerの関係と同じ)です。

```
Term> copy /sd/big.bin /flash/big.bin &   # バックグラウンドコピー
Term> worker 30 &                          # デモジョブ
Term> ps                                   # ジョブ一覧(進捗/スタック/経過時間)
Term> kill 3                               # 協調キャンセル
Term> kill -9 3                            # 強制kill(危険さも学べる)
```

- ジョブは `proc::spawn()` で起動され、進捗を報告し、完了すると**トースト通知**が画面に出る
- Music アプリの再生はジョブなので、**アプリを閉じても音楽が続く**
- SD/Flash へのアクセスは **FSミューテックス**で直列化(コピー中もFilesアプリが使える)
- ステータスバーに実行中ジョブ数のバッジを表示
